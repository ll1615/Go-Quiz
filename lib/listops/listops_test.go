...
func TestList(t *testing.T) {
	Convey("test list operation ", t, func() {
		...
		So(len(ListAnd(list1, list2)), ShouldEqual, 1)
		...
		So(len(ListAnd(list4, list5)), ShouldEqual, 3)

		So(len(ListMinus(list2, list6)), ShouldEqual, 3)
		So(len(ListAnd(list2, list6)), ShouldEqual, 2)

		So(len(ListMinus(list5, list6)), ShouldEqual, 3)
		So(len(ListAnd(list5, list6)), ShouldEqual, 3)
	})
}
